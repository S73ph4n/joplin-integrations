...
ENV = {"JOPLIN_TOKEN": "", "JOPLIN_HOST":"localhost", "JOPLIN_PORT":"41184", "NOTES_TAG":"email", "IMAP_SERVER": "", "IMAP_USER": "", "IMAP_PASSWORD": "", "WAIT_TIME":"60"}
    ...
    try:
        ...
        for msg in messages:
            if not CONFIRM or click.confirm(
                ...
                note = inbox_notebook.get_note_by_title(title, create_if_needed=True, allow_external_results=True)  # Create note in notebook (or find it if it exists)
                if note.body != "":
